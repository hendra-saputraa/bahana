...
const DashboardNewSong = () => {
  ...
  const saveArtist = () => {
    ...
    if (!artistImageCover || !artistName || !instagram || !youtube) {
      // throw allert
      dispatch({
        type: actionType.SET_ALERT_TYPE,
        alertType: "failed",
  ...
  return (
    <div className="flex flex-col items-center justify-center p-4 border border-gray-300 gap-4 rounded-md">
      ...
      <div className="flex w-full justify-between flex-wrap items-center gap-4">
        <FilterButtons filterData={allArtists} flag={"Artis"} />
        <FilterButtons filterData={allAlbums} flag={"Album"} />
        <FilterButtons filterData={filterByLanguage} flag={"Bahasa"} />
        <FilterButtons filterData={filters} flag={"Kategori"} />
      </div>
            ...
            {!songImageCover ? (
              ...
              <div className="relative w-full h-full overflow-hidden rounded-md">
                ...
                <button 
                  ...
                  className="absolute bottom-3 right-3 p-3 rounded-full bg-ungu text-xl cursor-pointer outline-none border-none hover:shadow-md hover:shadow-zinc-400 duration-200 transition-all ease-in-out"
                >
            ...
            {!audioImageCover ? (
              ...
              <div className="relative w-full h-full flex items-center justify-center overflow-hidden rounded-md">
                ...
                <button 
                  ...
                  className="absolute bottom-3 right-3 p-3 rounded-full bg-ungu text-xl cursor-pointer outline-none border-none hover:shadow-md duration-200 transition-all ease-in-out"
                >
                  <MdDelete className="text-white" />
                </button>
              </div>
      ...
      <div className="flex items-center justify-center w-60 cursor-pointer p-4">
        {isImageLoading || isAudioLoading ? (
          ...
          <motion.button 
            whileTap={{ scale : 0.75 }} 
            onClick={saveSong}
            className="px-8 py-2 w-full rounded-md text-white bg-ungu hover:shadow-lg"
          >
            Simpan Lagu
          </motion.button>
        )}
      ...
      {/* Artist Save */}
      <p className="text-xl font-semibold text-white">Tambah Artis</p>
      <div className="bg-card backdrop-blur-md w-full h-300 rounded-md border-2 border-dotted border-gray-300 cursor-pointer">
      ...
      <div className="flex items-center rounded-md p-3 border border-gray-300 w-full">
        ...
        <input 
          ...
          className="w-full text-base font-semibold text-gray-300 outline-none bg-transparent" 
          value={instagram} 
      ...
      {/* Album Save */}
      <p className="text-xl font-semibold text-white">Tambah Album</p>
      <div className="bg-card backdrop-blur-md w-full h-300 rounded-md border-2 border-dotted border-gray-300 cursor-pointer">
...
export const DisabledButton = () => {
  return (
    <button
      disabled
      type="button"
      className="text-white bg-card hover:bg-card focus:ring-4 focus:ring-card font-medium rounded-lg text-white text-sm px-5 py-2.5 text-center mr-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 inline-flex items-center"
    >
      <svg
        role="status"
...
export const FileLoader = ({ progress }) => {
  return (
    <div className="w-full h-full flex flex-col items-center justify-center">
      <p className="text-xl text-white font-semibold text-color">
        {Math.round(progress) > 0 && <>{`${Math.round(progress)}%`}</>}
      </p>
      <div className="w-20 h-20 min-w-[40px] bg-ungu animate-ping  rounded-full flex items-center justify-center relative">
        <div className="absolute inset-0 rounded-full bg-ungu blur-xl"></div>
      </div>
    </div>
  );
};
      ...
      <div className="flex flex-col items-center justify-center h-full">
        <div className="flex flex-col justify-center items-center cursor-pointer">
          ...
          <p className="text-lg text-white">
            Klik untuk upload {isImage ? "gambar" : "audio"}
          </p>
        </div>
      </div>
      <input 
